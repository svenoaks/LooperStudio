...
SuperpoweredAndroidAudioIO::SuperpoweredAndroidAudioIO(int samplerate, int buffersize, bool enableInput, bool enableOutput, audioProcessingCallback callback, void *clientdata, int latencySamples) {
    ...
    if (enableInput) { // Create the input buffer queue.
        ...
        SLDataFormat_PCM inputFormat = { SL_DATAFORMAT_PCM, static_cast<SLuint32>(2), static_cast<SLuint32>(samplerate * 1000),
            SL_PCMSAMPLEFORMAT_FIXED_16, SL_PCMSAMPLEFORMAT_FIXED_16, static_cast<SLuint32>(SL_SPEAKER_FRONT_LEFT | SL_SPEAKER_FRONT_RIGHT), SL_BYTEORDER_LITTLEENDIAN };
    ...
    if (enableOutput) { // Create the output buffer queue.
        ...
        SLDataFormat_PCM outputFormat = { SL_DATAFORMAT_PCM, static_cast<SLuint32>(2), static_cast<SLuint32>(samplerate * 1000),
            SL_PCMSAMPLEFORMAT_FIXED_16, SL_PCMSAMPLEFORMAT_FIXED_16, static_cast<SLuint32>(SL_SPEAKER_FRONT_LEFT | SL_SPEAKER_FRONT_RIGHT), SL_BYTEORDER_LITTLEENDIAN };
...
void SuperpoweredAndroidAudioIO::start() {
    if (internals->outputPlayInterface != NULL)
        (*internals->outputPlayInterface)->SetPlayState(internals->outputPlayInterface, SL_PLAYSTATE_PLAYING);
    if (internals->recordInterface != NULL)
            (*internals->recordInterface)->SetRecordState(internals->recordInterface, SL_RECORDSTATE_RECORDING);
}

void SuperpoweredAndroidAudioIO::pause() {
    if (internals->outputPlayInterface != NULL)
        (*internals->outputPlayInterface)->SetPlayState(internals->outputPlayInterface, SL_PLAYSTATE_PAUSED);
    if (internals->recordInterface != NULL)
            (*internals->recordInterface)->SetRecordState(internals->recordInterface, SL_RECORDSTATE_PAUSED);
}
